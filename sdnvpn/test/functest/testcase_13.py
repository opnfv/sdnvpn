...
def main():
    ...
    try:
        ...
        test_utils.update_router_extra_route(neutron_client,
                                             router_1_id,
                                             TESTCASE_CONFIG.extra_route_ip,
                                             vm_1_ip)
        # TODO: Openstack replaces the existing route with this new one
        # try to update extraroute with two nexthops in a single update call
        test_utils.update_router_extra_route(neutron_client,
                                             router_1_id,
                                             TESTCASE_CONFIG.extra_route_ip,
                                             vm_2_ip)
        ...
        instances_dhcp_up = test_utils.wait_for_instances_get_dhcp(vm_3)

        if (not instances_dhcp_up):
            logger.error("One or more instances are down")

        # TODO: find out how to get interface for extra route ip
        # under the vm1 and vm2
