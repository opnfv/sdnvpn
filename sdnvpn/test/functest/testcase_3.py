...
def main():
    ...
    if installer_type in ["fuel"]:
        controllers = [node for node in openstack_nodes
                       if "running" in node.run_cmd("sudo systemctl status "
                                                    "pendaylight")]
    elif installer_type in ["apex"]:
        controllers = [node for node in openstack_nodes
                       if "healthy" in node.run_cmd("sudo docker ps | "
                                                    "grep opendaylight_api")
                            if "Running" in node.run_cmd("sudo docker exec "
                             "opendaylight_api /opt/opendaylight/bin/status")]
