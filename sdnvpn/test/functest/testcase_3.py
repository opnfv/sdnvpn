...
def main():
    ...
    try:
        ...
        try:
            ...
            cmd_network = ('sudo /opt/quagga/bin/vtysh -c \'conf t\' '
                           '-c \'router bgp 100\' '
                           '-c \'address-family vpnv4\' '
                           '-c \'network {0} rd {1} tag 100\' '
                           '-c \'end\''.
                           format(TESTCASE_CONFIG.odl_network,
                                  TESTCASE_CONFIG.route_distinguishers))
            controller.run_cmd(cmd_network)

            results.get_route_exchange_status(quagga_vm,
                                              expected="PASS",
                                              timeout=50)

            fib = test_utils.check_odl_fib(controller, fip['fip_addr'],
                                           controller_ext_ip)
            if fib:
