...
def main():
    ...
    for comp in computes:
        if compute_node.host_ip in comp.run_cmd("ip a"):
            compute = comp
            break
    # Get the external IP of the compute where quagga is running
    # TODO check this works on apex
    cmd = "ip a | grep br-ex | grep inet | awk '{print $2}'"
    ext_cidr = compute.run_cmd(cmd).split("/")
    compute_ext_ip = ext_cidr[0]
    # Get the IP address of the external net by clearing the host bit
    ext_net_address = ext_cidr[0].split(".")
    ext_net_address[-1] = '0'
    ext_net_address = ".".join(ext_net_address)
