...
def main():
    ...
    try:
        ...
        kwargs = {
            ...
            "environment": env,
            "parameters": {
                "image_uuid": TESTCASE_CONFIG.image_name,
                "av_zone_1": az_1,
                "av_zone_2": az_2
            }
        }
        ...
        while attempts < 12:
            ...
            stack_st = os_utils.list_stack(heat_client, **kwargs).next().stack_status
        ...
        logger.debug("Stack status checks: %s" % attempts)
        ...
        logger.debug("value of the output: '%s'" % net_1_output)
        n1_o = net_1_output['output']
        network_1_id = n1_o['output_value']
        logger.debug("Network 1 '%s' read from heat output" % network_1_id)
        net_2_output = os_utils.get_output(heat_client, stack_id, 'net_2_o')
        n2_o = net_2_output['output']
        network_2_id = n2_o['output_value']
        logger.debug("Network 2 '%s' read from heat output" % network_2_id)
        ...
        logger.debug("Stack status checks: %s" % attempts)
        if stack_delete_complete is False:
