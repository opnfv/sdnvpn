...
def download_and_add_image_on_glance(cloud, image_name, image_url, data_dir):
    try:
        dest_path = data_dir
    ...
    try:
        image = create_glance_image(
            cloud, image_name, dest_path + file_name)
        if not image:
            return False
        else:
            return image
...
def get_image_id(cloud, image_name):
    images = cloud.image.images()
    id = ''
    for i in images:
...
def create_glance_image(cloud,
    ...
    if not os.path.isfile(file_path):
        ...
        return None
    try:
        image_id = get_image_id(cloud, image_name)
        if image_id != '':
            ...
            with open(file_path) as image_data:
                image = cloud.image.upload_image(name=image_name,
                                                 is_public=public,
                                                 disk_format=disk,
...
def delete_glance_image(cloud, image_id):
    try:
        cloud.image.delete_image(image_id)
        return True
    except Exception as e:
