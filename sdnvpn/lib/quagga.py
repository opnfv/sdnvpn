...
from sdnvpn.lib.utils import exec_odl_command, exec_cmd
...
def odl_add_neighbor(neighbor_ip, own_ip):
    command = 'configure-bgp -op add-neighbor --as-num 200'
    command += ' --ip %s --use-source-ip %s' % (neighbor_ip, own_ip)
    _, success = exec_odl_command(own_ip, command)
    return success
...
def check_for_peering(controller_ip):
    ...
    while not neighbors or 'Established' not in neighbors and tries > 0:
        neighbors, _ = exec_odl_command(controller_ip, cmd, verbose=False)
