...
def generate_ping_userdata(ips_tuple):
    return ("#!/bin/bash\n"
            "ips=(%s %s %s %s)\n"
            "while true; do\n"
            "for ip in ${ips[@]}; do\n"
...
def main():
    ...
    vm_2 = create_instance(nova_client,
                           ...
                           sg_id,
                           compute_node_1)
    vm_2_ip = vm_2.networks.get(network_1_id)[0]
    ...
    vm_3 = create_instance(nova_client,
                           ...
                           sg_id,
                           compute_node_2)
    vm_3_ip = vm_3.networks.get(network_1_id)[0]
    logger.info("Instance '%s' booted successfully. IP='%s'." %
    ...
    vm_5 = create_instance(nova_client,
                           ...
                           sg_id,
                           compute_node_2)
    vm_5_ip = vm_5.networks.get(network_2_id)[0]
    logger.info("Instance '%s' booted successfully. IP='%s'." %
    ...
    vm_4 = create_instance(nova_client,
                           ...
                           compute_node_1,
                           userdata=u4)
    vm_4_ip = vm_4.networks.get(network_2_id)[0]
    logger.info("Instance '%s' booted successfully. IP='%s'." %
    ...
    vm_1 = create_instance(nova_client,
                           ...
                           compute_node_1,
                           userdata=u1)
    vm_1_ip = vm_1.networks.get(network_1_id)[0]
    logger.info("Instance '%s' booted successfully. IP='%s'." %
