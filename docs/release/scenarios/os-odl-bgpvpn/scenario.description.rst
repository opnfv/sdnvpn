...
APEX installer configuration and BGPVPN depolyment
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

To install the SDNVPN feature using the APEX installer, follow the `APEX installation guide <(https://wiki.
opnfv.org/display/apex/Integration+Guide)>`_ . When prompted activate the SDNVPN feature base on openstack configuration:
