...
class DeploymentCloner(Service):
    ...
    def run(self, sys_args, config):
        dest_dir = sys_args.dest_dir if sys_args.dest_dir[:-1] == '/' else (sys_args.dest_dir + '/')
        ...
        underlcloud.copy('from', dest_dir, self.undercloud_root_dir + '/cloner-info/')
        ...
        for name, node in node_yaml['servers'].iteritems():
            node['vNode-name'] = self.get_virtual_node_name_from_mac(node['orig-ctl-mac'])
    ...
    def get_virtual_node_name_from_mac(self, mac):
        ...
        for node in vNode_names.split('\n'):
            if 'baremetal' in node:
                admin_net_mac, _ = execute('virsh domiflist %s |grep admin |awk \'{print $5}\'' % node,
                                        shell=True)
                if admin_net_mac.replace('\n', '') == mac:
                    return node
        raise Exception('Could not find corresponding virtual node for MAC: %s' % mac)
