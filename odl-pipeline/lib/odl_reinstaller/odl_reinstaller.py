...
class ODLReInstaller(Service):
    ...
    def run(self, sys_args, config):
        ...
        for node in self.nodes:
            if 'ODL' in node.config:
                ...
                if node.config['ODL'].get('dir_exsist'):
                    ...
                    node.copy('to', sys_args.odl_artifact, '/tmp/odl-artifact/' + tarball_name)
                    node.execute('rm -rf /opt/opendaylight/*', as_root=True)
                    node.execute('mkdir -p /opt/opendaylight/*', as_root=True)
                    LOG.info('Extracting %s to /opt/opendaylight/ on node %s' % (tarball_name, node.name))
                    node.execute('tar -zxf %s --strip-components=1 -C /opt/opendaylight/' % (tar_tmp_path + tarball_name), as_root=True)
                ...
                if node.config['ODL'].get('active'):
                    ...
                    node.copy('to', 'odl_reinstaller/install_odl.pp', tar_tmp_path)
                    node.execute('puppet apply --modulepath=/etc/puppet/modules/ %sinstall_odl.pp '
                                 '--verbose --debug --trace --detailed-exitcodes' % tar_tmp_path,
        ...
        for node in self.nodes:
            ...
            if ovs_controller:
                node.execute('ovs-vsctl set-controller br-int ' + ovs_controller, as_root=True)

    def create_cli_parser(self, parser):
        parser.add_argument('-c', '--config', help="Give the path to the node config file (node.yaml)",
                            required=True)
        parser.add_argument('--odl-artifact', help=("Path to Opendaylight tarball"),
                            required=True)
        parser.add_argument('--id-rsa', help=("Path to the identity file which can "
