...
class TripleOManager(Service):
    ...
    def prepare_for_ci_pipeline(self):
        ...
        for node in node_manager.get_nodes():
            ...
            rv, _ = node.execute('ps aux |grep -v grep |grep karaf', as_root=True , check_exit_code=[0, 1])
            if 'java' in rv:
                self.node_info['servers'][node.name]['ODL']['active'] = True

            if node.is_dir('/opt/opendaylight') or node.is_file('/opt/opendaylight-was-there'):
                self.node_info['servers'][node.name]['ODL']['dir_exsist'] = True
            ...
            rv, _ = node.execute('ovs-vsctl get-controller br-int', as_root=True)
            self.node_info['servers'][node.name]['ovs-controller'] = rv.replace('\n', '')
    ...
    def gen_env_info(self, sys_args, config):
        ...
        # copy ssh key
        shutil.copy('to', '/home/stack/.ssh/id_rsa', sys_args.out + '/undercloud_ssh/')
        shutil.copy('to', '/home/stack/.ssh/id_rsa.pub', sys_args.out + '/undercloud_ssh/')
        # copy rc files 
    ...
    def gen_node_info(self):
        ...
        for server in self.novacl.servers.list():
            if 'overcloud-controller' in server.name:
                ...
                raise Exception('Unknown type (controller/compute) %s ' % server.name)
            ctlplane_mac = None
            for interface in server.interface_list():
                if interface.net_id == ctlplane_id:
                    ctlplane_mac = interface.mac_addr
            if not ctlplane_mac:
                raise Exception('Could not find mac address for ctl-plane for server %s' % server.name)
            self.node_info['servers'][server.name] = {'address': self.get_address_of_node(server=server),
                                                      'user': 'heat-admin',
                                                      'type': type,
    ...
    def create_auth(self):
        ...
        if not self.auth:
            self.auth = identity.Password(auth_url=os.environ['OS_AUTH_URL'],
    ...
    def get_node_name_by_ilo_address(self, ilo_address):
        try:
            ...
            for node in self.ironiccl.node.list():
                ...
                if ilo_address == self.ironiccl.node.get_by_instance_uuid(nova_uuid).driver_info['ilo_address']:
